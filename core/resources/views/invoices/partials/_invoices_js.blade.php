    ...
    $( document ).on('click', '.deleteItem', function() {
        ...
        $('#invoice_form').validator().on('submit', function (e) {
            ...
            if (!e.isDefaultPrevented()) {
                ...
                var data = $('#invoice_form select, input, textarea').not('#product, #tax, #quantity, #rate, #itemId').serializeArray();
...
function calcTotals(){
    ...
    total           += parseFloat(subTotal+totalTax-discount);

    amountDue       += parseFloat(subTotal+totalTax-discount-paid);
